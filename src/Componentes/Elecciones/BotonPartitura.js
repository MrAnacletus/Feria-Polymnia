...
class BotonPartitura extends Component {
    ...
    render() {
        ...
        if (this.state.partitura === "si"){
            return (
                <button type="button" className="btn btn-dark" disabled={!this.state.habilitado} onClick={() => this.props.elegirEsteInstrumento(this.state.instrumento.nombre, this.state.partitura,"a")}>
                    Partitura
                </button>
            );
            ...
            if (this.state.habilitado == "si" && this.props.melodia === "no"){
                if (this.state.instrumento.compatibles.length == 0){
                    return(
                        <button type="button" className="btn btn-dark" onClick={() => this.props.elegirEsteInstrumento(this.state.instrumento.nombre,"no","a")}>
                            Tablatura
                        </button>
                    );
                }else{
                ...
                return (
                    <button type="button" className="btn btn-dark" onClick={() => this.props.elegirEsteInstrumento(this.state.instrumento.nombre,"no","a")} disabled={true}>
                        Tablatura
                    </button>
                )
            }
